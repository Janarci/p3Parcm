...
objLoader::objLoader(GLuint modelTransformLoc, IExecutionEvent* executionEvent)
{
...
//
		...
		/*if (true)
		{
			glBindVertexArray(SwordObjData2.vaoId);
			glBindTexture(GL_TEXTURE_2D, texture[5]);
			glUniformMatrix4fv(modelTransformLoc, 1, GL_FALSE, glm::value_ptr(transSword));
			glDrawElements(GL_TRIANGLES, SwordObjData2.numFaces, GL_UNSIGNED_INT, (void*)0);
		}
		*/
...
bool objLoader::FileExists_(const std::string& absFilename)
{
	struct stat buffer;
	return (stat(absFilename.c_str(), &buffer) == 0);
}

void objLoader::LoadTextureData_(ObjData* objData, GLuint* texture, GLuint* texture2, int num)
{
	int width, height;
...
void objLoader::LoadObjToMemory_(ObjData* objData, GLfloat scaleFactor, GLfloat tOffset[], GLuint* texture,
